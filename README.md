...
---
## 初期セットアップ (環境変数)
本リポジトリのコードはセキュリティを考慮し、各種APIキーやトークンをプレースホルダー化（または環境変数化）しています。実環境で動作させる場合は、以下の設定を行ってください。

**1. フロントエンド (Wagtail_app_v2.jsx)**
Firebaseの接続情報および外部APIのURLを、コード内のプレースホルダー（`YOUR_FIREBASE_API_KEY` 等）に設定するか、`.env` ファイルを作成して読み込ませてください。

**2. バックエンド (GAS)**
LINE Messaging APIのチャネルアクセストークンや、Firebaseの秘密鍵（Service Account Key）は、ソースコード（`.gs` ファイル）には直接記述せず、必ず **GASの「スクリプトプロパティ」** に登録してセキュアに呼び出す仕様としています。
## System Architecture
